    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="overflow-hidden border-[var(--border-subtle)] bg-surface text-text-primary sm:max-w-2xl">
        ...
        <DialogHeader className="rounded-2xl border border-border bg-[var(--sidebar-hover)] p-5">
          ...
          <DialogTitle className="text-2xl tracking-tight text-text-primary">Changer le statut</DialogTitle>
          <DialogDescription>
            Mets a jour l'etat logistique de {pc?.hostname || 'ce poste'}.
          </DialogDescription>
          <div className="mt-4 inline-flex w-fit rounded-full border border-border bg-surface px-3 py-1.5 text-xs text-text-secondary">
        ...
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
          ...
          {isSentStatus && (
            <>
              <div className="space-y-2">
                <Label htmlFor="pc-status-site" className="text-xs uppercase tracking-[0.24em] text-text-muted">Envoye vers</Label>
                <div className="relative">
                  <Send className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-brand-light/70" />
                  <Input id="pc-status-site" value={form.sentTo} onChange={(event) => setForm((current) => ({ ...current, sentTo: event.target.value }))} className="h-12 rounded-2xl border-border bg-surface pl-10 text-text-primary" />
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="pc-status-recipient" className="text-xs uppercase tracking-[0.24em] text-text-muted">Destinataire</Label>
                <div className="relative">
                  <UserRound className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-brand-light/70" />
                  <Input id="pc-status-recipient" value={form.sentRecipient} onChange={(event) => setForm((current) => ({ ...current, sentRecipient: event.target.value }))} className="h-12 rounded-2xl border-border bg-surface pl-10 text-text-primary" />
                </div>
              </div>
            </>
          )}
          <div className="space-y-2 md:col-span-2">
            <Label htmlFor="pc-status-notes" className="text-xs uppercase tracking-[0.24em] text-text-muted">Notes</Label>
            <div className="relative">
              <NotebookPen className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-brand-light/70" />
              <textarea id="pc-status-notes" value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} className={cn('min-h-[140px] w-full rounded-2xl border border-border bg-surface px-10 py-3 text-sm text-text-primary outline-none transition focus:ring-2', activeTheme.ring)} />
            </div>
          </div>
